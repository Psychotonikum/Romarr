...
namespace Romarr.Common.EnvironmentInfo
{
    ...
    public class AppFolderFactory : IAppFolderFactory
    {
        ...
        public void Register()
        {
            ...
            {
                throw new RomarrStartupException("Cannot create AppFolder, Access to the path {0} is denied", _appFolderInfo.AppDataFolder);
            }
            ...
            {
                throw new RomarrStartupException("AppFolder {0} is not writable", _appFolderInfo.AppDataFolder);
            }
        ...
        private void MigrateAppDataFolder()
        {
            try
            {
                if (OsInfo.IsOsx)
                {
                    var userAppDataFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile, Environment.SpecialFolderOption.DoNotVerify), ".config", "Romarr");
                ...
                var oldDbFile = Path.Combine(_appFolderInfo.AppDataFolder, "romarr.db");
            ...
            catch (Exception ex)
            {
                ...
                throw new RomarrStartupException(ex, "Unable to migrate AppData folder from {0} to {1}. Migrate manually", _appFolderInfo.LegacyAppDataFolder, _appFolderInfo.AppDataFolder);
            }
        }
        ...
        private void RemovePidFile()
        {
            ...
            {
                _diskProvider.DeleteFile(Path.Combine(_appFolderInfo.AppDataFolder, "romarr.pid"));
            }
        }
    }
}
