...
namespace Romarr.Common.EnvironmentInfo
{
    public class RuntimeInfo : IRuntimeInfo
    {
        ...
        public RuntimeInfo(Logger logger, IOsInfo osInfo, IHostLifetime hostLifetime = null)
        {
            ...
            // net6.0 will return Romarr.dll for entry assembly, we need the actual
            // executable name (Romarr on linux).  On mono this will return the location of
            ...
            if (entry != null)
            {
                ...
                IsWindowsTray = OsInfo.IsWindows && entry.ModuleName == $"{ProcessProvider.ROMARR_PROCESS_NAME}.exe";
            }
        }
        ...
        public bool IsTray
        {
            get
            {
                if (OsInfo.IsWindows)
                {
                    return IsUserInteractive && Process.GetCurrentProcess().ProcessName.Equals(ProcessProvider.ROMARR_PROCESS_NAME, StringComparison.InvariantCultureIgnoreCase);
                }
