...
{
    public static class RomarrLogger
    {
        ...
        private static void RegisterSentry(bool updateClient, IAppFolderInfo appFolderInfo)
        {
            ...
            catch (Exception ex)
            {
                ...
                LogManager.GetLogger(nameof(RomarrLogger)).Debug(ex, "Failed to load dependency, may need an OS update");
        ...
        private static void RegisterConsole()
        {
            ...
            var logFormat = Enum.TryParse<ConsoleLogFormat>(Environment.GetEnvironmentVariable("ROMARR__LOG__CONSOLEFORMAT"), out var formatEnumValue)
                ? formatEnumValue
        ...
        {
            RegisterAppFile(appFolderInfo, "appFileInfo", "romarr.txt", 5, LogLevel.Info);
            RegisterAppFile(appFolderInfo, "appFileDebug", "romarr.debug.txt", 50, LogLevel.Off);
            RegisterAppFile(appFolderInfo, "appFileTrace", "romarr.trace.txt", 50, LogLevel.Off);
        }
        ...
        private static void RegisterGlobalFilters()
        {
            LogManager.Setup().LoadConfiguration(c =>
            {
                ...
                c.ForLogger("Romarr.Http.Authentication.ApiKeyAuthenticationHandler").WriteToNil(LogLevel.Info);
            });
        }

        public static Logger GetLogger(Type obj)
        {
            return LogManager.GetLogger(obj.Name.Replace("Romarr.", ""));
        }
        ...
        public static void ConfigureConsoleLayout(ColoredConsoleTarget target, ConsoleLogFormat format)
        {
            ...
            {
                ConsoleLogFormat.Clef => RomarrLogger.ClefLogLayout,
                _ => RomarrLogger.CleansingConsoleLayout
            };
        }
