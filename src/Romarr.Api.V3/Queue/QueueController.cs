...
{
    [V3ApiController]
    public class QueueController : RestControllerWithSignalR<QueueResource, Romarr.Core.Queue.Queue>,
                               IHandle<ObsoleteQueueUpdatedEvent>, IHandle<PendingReleasesUpdatedEvent>
    {
        ...
        public object RemoveMany([FromBody] QueueBulkResource resource, [FromQuery] bool removeFromClient = true, [FromQuery] bool blocklist = false, [FromQuery] bool skipRedownload = false, [FromQuery] bool changeCategory = false)
        {
            ...
            var pendingToRemove = new List<Romarr.Core.Queue.Queue>();
        ...
        public PagingResource<QueueResource> GetQueue([FromQuery] PagingRequestResource paging, bool includeUnknownSeriesItems = false, bool includeSeries = false, bool includeGameFile = false, [FromQuery] int[] gameIds = null, DownloadProtocol? protocol = null, [FromQuery] int[] languages = null, [FromQuery] int[] quality = null, [FromQuery] QueueStatus[] status = null)
        {
            var pagingResource = new PagingResource<QueueResource>(paging);
            var pagingSpec = pagingResource.MapToPagingSpec<QueueResource, Romarr.Core.Queue.Queue>(
            ...
            return pagingSpec.ApplyToPage((spec) => GetQueue(spec, gameIds?.ToHashSet(), protocol, languages?.ToHashSet(), quality?.ToHashSet(), status?.ToHashSet(), includeUnknownSeriesItems), (q) => MapToResource(q, includeSeries, includeGameFile));
        }

        private PagingSpec<Romarr.Core.Queue.Queue> GetQueue(PagingSpec<Romarr.Core.Queue.Queue> pagingSpec, HashSet<int> gameIds, DownloadProtocol? protocol, HashSet<int> languages, HashSet<int> quality, HashSet<QueueStatus> status, bool includeUnknownSeriesItems)
        {
            ...
            IOrderedEnumerable<Romarr.Core.Queue.Queue> ordered;
        ...
        private Func<Romarr.Core.Queue.Queue, object> GetOrderByFunc(PagingSpec<Romarr.Core.Queue.Queue> pagingSpec)
        {
        ...
        private void Remove(Romarr.Core.Queue.Queue pendingRelease, bool blocklist)
        {
            if (blocklist)
            {
        ...
        private QueueResource MapToResource(Romarr.Core.Queue.Queue queueItem, bool includeSeries, bool includeGameFile)
        {
            return queueItem.ToResource(includeSeries, includeGameFile);
        }
