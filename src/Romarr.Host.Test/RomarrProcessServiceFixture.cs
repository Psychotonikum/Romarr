...
{
    [TestFixture]
    public class RomarrProcessServiceFixture : TestBase<SingleInstancePolicy>
    {
        ...
        public void Setup()
        {
            ...
            Mocker.GetMock<IProcessProvider>()
                  .Setup(s => s.FindProcessByName(ProcessProvider.ROMARR_CONSOLE_PROCESS_NAME))
                  .Returns(new List<ProcessInfo>());

            Mocker.GetMock<IProcessProvider>()
                  .Setup(s => s.FindProcessByName(ProcessProvider.ROMARR_PROCESS_NAME))
                  .Returns(new List<ProcessInfo>());
        }
        ...
        {
            Mocker.GetMock<IProcessProvider>()
                  .Setup(c => c.FindProcessByName(It.Is<string>(f => f.Contains("Romarr"))))
                  .Returns(new List<ProcessInfo>
                           {
        ...
        {
            Mocker.GetMock<IProcessProvider>()
                  .Setup(c => c.FindProcessByName(ProcessProvider.ROMARR_CONSOLE_PROCESS_NAME))
                  .Returns(new List<ProcessInfo>
                           {
        ...
        {
            Mocker.GetMock<IProcessProvider>()
                  .Setup(c => c.FindProcessByName(ProcessProvider.ROMARR_PROCESS_NAME))
                  .Returns(new List<ProcessInfo>
                           {
                               new ProcessInfo { Id = CURRENT_PROCESS_ID },
                               new ProcessInfo { Id = 10 }
