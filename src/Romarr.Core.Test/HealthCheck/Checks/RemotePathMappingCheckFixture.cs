...
namespace Romarr.Core.Test.HealthCheck.Checks
{
    ...
    public class RemotePathMappingCheckFixture : CoreTest<RemotePathMappingCheck>
    {
        ...
        [Test]
        public void should_return_ok_on_gameFile_imported_event()
        {
            GivenFolderExists(_downloadRootPath);
            var importEvent = new FileImportedEvent(new LocalGameFile(), new RomFile(), new List<DeletedRomFile>(), true, new DownloadClientItem());

            Subject.Check(importEvent).ShouldBeOk();
        }

        [Test]
        public void should_return_permissions_error_on_gameFile_import_failed_event_if_file_exists()
        {
            ...
            var importEvent = new GameFileImportFailedEvent(new Exception(), localRom, true, new DownloadClientItem());

            Subject.Check(importEvent).ShouldBeError(wikiFragment: "permissions-error");
        }

        [Test]
        public void should_return_permissions_error_on_gameFile_import_failed_event_if_folder_exists()
        {
            GivenFolderExists(_downloadItemPath);

            var importEvent = new GameFileImportFailedEvent(null, null, true, _downloadItem);

            Subject.Check(importEvent).ShouldBeError(wikiFragment: "permissions-error");
        }

        [Test]
        public void should_return_permissions_error_on_gameFile_import_failed_event_for_local_client_if_folder_does_not_exist()
        {
            var importEvent = new GameFileImportFailedEvent(null, null, true, _downloadItem);
        ...
        [Test]
        public void should_return_mapping_error_on_gameFile_import_failed_event_for_remote_client_if_folder_does_not_exist()
        {
            _clientStatus.IsLocalhost = false;
            var importEvent = new GameFileImportFailedEvent(null, null, true, _downloadItem);
        ...
        [Test]
        public void should_return_mapping_error_on_gameFile_import_failed_event_for_remote_client_if_path_invalid()
        {
            _clientStatus.IsLocalhost = false;
        ...
        [Test]
        public void should_return_download_client_error_on_gameFile_import_failed_event_for_remote_client_if_path_invalid()
        {
            _clientStatus.IsLocalhost = true;
            _downloadItem.OutputPath = new OsPath("an invalid path");
            var importEvent = new GameFileImportFailedEvent(null, null, true, _downloadItem);
        ...
        [Test]
        public void should_return_docker_mapping_error_on_gameFile_import_failed_event_inside_docker_if_folder_does_not_exist()
        {
            GivenDocker();
