...
using Romarr.Core.Download.Clients.Sabnzbd;
using Romarr.Core.Download.Clients.Sabnzbd.Responses;
using Romarr.Core.RemotePathMappings;
using Romarr.Core.Games;
using Romarr.Core.Validation;
using Romarr.Test.Common;

namespace Romarr.Core.Test.Download.DownloadClientTests.SabnzbdTests
{
    [TestFixture]
    public class SabnzbdFixture : DownloadClientFixtureBase<Sabnzbd>
    {
        ...
        [Test]
        public async Task Download_should_use_sabRecentTvPriority_when_recentGameFile_is_true()
        {
            Mocker.GetMock<ISabnzbdProxy>()
                    .Setup(s => s.DownloadNzb(It.IsAny<byte[]>(), It.IsAny<string>(), It.IsAny<string>(), (int)SabnzbdPriority.High, It.IsAny<SabnzbdSettings>()))
                    .Returns(new SabnzbdAddResponse { Ids = new List<string> { "romarrtest" } });
        ...
        public void should_test_develop_version_successfully()
        {
            ...
            var result = new RomarrValidationResult(Subject.Test());
