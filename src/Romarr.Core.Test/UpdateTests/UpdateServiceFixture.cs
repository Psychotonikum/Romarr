...
namespace Romarr.Core.Test.UpdateTests
{
    [TestFixture]
    public class UpdateServiceFixture : CoreTest<InstallUpdateService>
    {
        ...
        public void Setup()
        {
            if (OsInfo.IsLinux)
            {
                ...
                {
                    FileName = "Romarr.develop.2.0.0.0.tar.gz",
                    Url = "http://download.romarr.tv/v2/develop/mono/Romarr.develop.tar.gz",
            ...
            else
            {
                _updatePackage = new UpdatePackage
                {
                    FileName = "Romarr.develop.2.0.0.0.zip",
                    Url = "http://download.romarr.tv/v2/develop/windows/Romarr.develop.zip",
                    Version = new Version("2.0.0.0")
                };
            }

            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.TempFolder).Returns(TempFolder);
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.StartUpFolder).Returns(@"C:\Romarr".AsOsAgnostic);
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.AppDataFolder).Returns(@"C:\ProgramData\Romarr".AsOsAgnostic);
            ...
            Mocker.GetMock<IRuntimeInfo>().Setup(c => c.ExecutingApplication).Returns(@"C:\Test\Romarr.exe");
            ...
            Mocker.GetMock<IDiskProvider>()
                  .Setup(v => v.FileExists(It.Is<string>(s => s.EndsWith("Romarr.Update".ProcessNameToExe()))))
                  .Returns(true);
        ...
        {
            const string scriptPath = "/tmp/romarr/update.sh";
        ...
        public void Should_download_and_extract_to_temp_folder()
        {
            ...
            updateSubFolder.GetDirectories("Romarr").Should().HaveCount(1);
        ...
        {
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.StartUpFolder).Returns(@"C:\Romarr".AsOsAgnostic);
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.AppDataFolder).Returns(@"C:\Romarr\AppData".AsOsAgnostic);
        ...
        {
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.StartUpFolder).Returns(@"C:\Romarr".AsOsAgnostic);
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.AppDataFolder).Returns(@"C:\Romarr".AsOsAgnostic);
