...
{
    [TestFixture]
    public class PlatformSearchFixture : TestBase<TestIndexer>
    {
        ...
        private void WithIndexer(bool paging, int resultCount)
        {
            ...
            requestGenerator.Setup(s => s.GetSearchRequests(It.IsAny<PlatformSearchCriteria>()))
                .Returns(pageable);
        ...
        public void should_not_use_offset_if_result_count_is_less_than_90()
        {
            ...
            Subject.Fetch(new PlatformSearchCriteria { Game = _series, SceneTitles = new List<string> { _series.Title } });
