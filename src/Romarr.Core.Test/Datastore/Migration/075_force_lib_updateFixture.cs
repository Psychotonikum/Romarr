    ...
    public class force_lib_updateFixture : MigrationTest<force_lib_update>
    {
        ...
        public void should_reset_job_last_execution_time()
        {
            var db = WithMigrationTestDb(c =>
            {
                ...
                {
                    TypeName = "Romarr.Core.Games.Commands.RefreshSeriesCommand",
                    Interval = 10,
                ...
                c.Insert.IntoTable("ScheduledTasks").Row(new
                {
                    TypeName = "Romarr.Core.Backup.BackupCommand",
                    Interval = 10,
                    LastExecution = "2000-01-01 00:00:00"
                });
            });
            ...
            jobs.Single(c => c.TypeName == "Romarr.Core.Games.Commands.RefreshSeriesCommand")
                .LastExecution.Year.Should()
                .Be(2014);

            jobs.Single(c => c.TypeName == "Romarr.Core.Backup.BackupCommand")
               .LastExecution.Year.Should()
               .Be(2000);
        }
