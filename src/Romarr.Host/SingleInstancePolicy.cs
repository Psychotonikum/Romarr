...
namespace Romarr.Host
{
    ...
    public class SingleInstancePolicy : ISingleInstancePolicy
    {
        ...
        public void PreventStartIfAlreadyRunning()
        {
            if (IsAlreadyRunning())
            {
                _logger.Warn("Another instance of Romarr is already running.");
        ...
        {
            foreach (var processId in GetOtherRomarrProcessIds())
            {
        ...
        public void WarnIfAlreadyRunning()
        {
            if (IsAlreadyRunning())
            {
                _logger.Debug("Another instance of Romarr is already running.");
            }
        }

        private bool IsAlreadyRunning()
        {
            return GetOtherRomarrProcessIds().Any();
        }

        private List<int> GetOtherRomarrProcessIds()
        {
            try
            {
                var currentId = _processProvider.GetCurrentProcess().Id;

                var otherProcesses = _processProvider.FindProcessByName(ProcessProvider.ROMARR_CONSOLE_PROCESS_NAME)
                                                     .Union(_processProvider.FindProcessByName(ProcessProvider.ROMARR_PROCESS_NAME))
                                                     .Select(c => c.Id)
                ...
                {
                    _logger.Info("{0} instance(s) of Romarr are running", otherProcesses.Count);
                }
            ...
            {
                _logger.Warn(ex, "Failed to check for multiple instances of Romarr.");
                return new List<int>();
            }
        }
    }
