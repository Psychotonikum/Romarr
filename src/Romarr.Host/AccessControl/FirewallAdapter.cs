...
namespace Romarr.Host.AccessControl
{
    ...
    public class FirewallAdapter : IFirewallAdapter
    {
        ...
        public void MakeAccessible()
        {
            ...
            {
                if (!IsRomarrPortOpen(_configFileProvider.Port))
                {
                    _logger.Debug("Opening Port for Romarr: {0}", _configFileProvider.Port);
                    OpenFirewallPort(_configFileProvider.Port);
                }

                if (_configFileProvider.EnableSsl && !IsRomarrPortOpen(_configFileProvider.SslPort))
                {
                    _logger.Debug("Opening SSL Port for Romarr: {0}", _configFileProvider.SslPort);
                    OpenFirewallPort(_configFileProvider.SslPort);
                }
            }
        }

        private bool IsRomarrPortOpen(int port)
        {
            try
            {
        ...
        private void OpenFirewallPort(int portNumber)
        {
            ...
            {
                _logger.Warn(ex, "Failed to open port in firewall for Romarr " + portNumber);
            }
        }
