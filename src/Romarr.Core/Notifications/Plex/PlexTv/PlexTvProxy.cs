...
namespace Romarr.Core.Notifications.Plex.PlexTv
{
    ...
    public class PlexTvProxy : IPlexTvProxy
    {
        ...
        private string ProcessRequest(HttpRequestBuilder requestBuilder)
        {
            ...
            {
                throw new RomarrClientException(ex.Response.StatusCode, "Unable to connect to plex.tv");
            }
            catch (WebException)
            {
                throw new RomarrClientException(HttpStatusCode.BadRequest, "Unable to connect to plex.tv");
            }
