...
namespace Romarr.Core.Backup
{
    ...
    public class BackupService : IBackupService, IExecute<BackupCommand>
    {
        ...
        public static readonly Regex BackupFileRegex = new Regex(@"(romarr|romarr)_backup_(v[0-9.]+_)?[._0-9]+\.zip", RegexOptions.Compiled | RegexOptions.IgnoreCase);
                             ...
                             Logger logger)
        {
            ...
            _backupTempFolder = Path.Combine(_appFolderInfo.TempFolder, "romarr_backup");
        }
        ...
        public void Backup(BackupType backupType)
        {
            ...
            var backupFilename = $"romarr_backup_v{BuildInfo.Version}_{dateNow:yyyy.MM.dd_HH.mm.ss}.zip";
            ...
            _diskProvider.DeleteFile(Path.Combine(_backupTempFolder, "romarr.db-journal"));
        ...
        public void Restore(string backupFileName)
        {
            if (backupFileName.EndsWith(".zip"))
            {
                ...
                var temporaryPath = Path.Combine(_appFolderInfo.TempFolder, "romarr_backup_restore");
                ...
                foreach (var file in _diskProvider.GetFiles(temporaryPath, false))
                {
                    ...
                    if (fileName.Equals("romarr.db", StringComparison.InvariantCultureIgnoreCase))
                    {
