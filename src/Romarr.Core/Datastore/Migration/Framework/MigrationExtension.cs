...
namespace Romarr.Core.Datastore.Migration.Framework
{
    public static class MigrationExtension
    {
        ...
        public static IMigrationRunnerBuilder AddRomarrSQLite(this IMigrationRunnerBuilder builder, bool binaryGuid = false, bool useStrictTables = false)
        {
            builder.Services
                .AddTransient<SQLiteBatchParser>()
                .AddScoped<SQLiteDbFactory>()
                .AddScoped<RomarrSQLiteProcessor>(sp =>
                {
                    var factory = sp.GetService<SQLiteDbFactory>();
                    var logger = sp.GetService<ILogger<RomarrSQLiteProcessor>>();
                    ...
                    return new RomarrSQLiteProcessor(factory, sp.GetService<SQLiteGenerator>(), logger, options, connectionStringAccessor, sp, sqliteQuoter);
                })
                .AddScoped<ISQLiteTypeMap>(_ => new RomarrSQLiteTypeMap(useStrictTables))
                .AddScoped<IMigrationProcessor>(sp => sp.GetRequiredService<RomarrSQLiteProcessor>())
                .AddScoped(
                    sp =>
                    {
