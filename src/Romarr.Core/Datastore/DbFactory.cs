...
namespace Romarr.Core.Datastore
{
    ...
    {
        private static readonly Logger Logger = RomarrLogger.GetLogger(typeof(DbFactory));
        ...
        private void CreateMain(string connectionString, MigrationContext migrationContext, DatabaseType databaseType)
        {
            ...
            catch (SQLiteException e)
            {
                ...
                if (OsInfo.IsOsx)
                {
                    throw new CorruptDatabaseException("Database file: {0} is corrupt, restore from backup if available. See: https://wiki.servarr.com/romarr/faq#i-use-romarr-on-a-mac-and-it-suddenly-stopped-working-what-happened", e, fileName);
                }

                throw new CorruptDatabaseException("Database file: {0} is corrupt, restore from backup if available. See: https://wiki.servarr.com/romarr/faq#i-am-getting-an-error-database-disk-image-is-malformed", e, fileName);
            }
                ...
                if (e.InnerException is SocketException)
                {
                    ...
                    while (true)
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            throw new RomarrStartupException(ex, "Error creating main database");
                        }
                    }
                }
                else
                {
                    throw new RomarrStartupException(e, "Error creating main database");
                }
            }
            catch (Exception e)
            {
        ...
        private void CreateLog(string connectionString, MigrationContext migrationContext, DatabaseType databaseType)
        {
            ...
            {
                throw new RomarrStartupException(e, "Error creating log database");
            }
        }
    }
}
