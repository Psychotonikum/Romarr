    ...
    public class ConnectionStringFactory : IConnectionStringFactory
    {
        ...
        public ConnectionStringFactory(IAppFolderInfo appFolderInfo, IConfigFileProvider configFileProvider)
        {
            ...
            switch (connectionStringType)
            {
                ...
                case ConnectionStringType.Sqlite:
                    ...
                    break;
                default:
                    throw new RomarrStartupException("Unable to determine database connection string for type {0}.", connectionStringType.ToString());
            }
        }
        ...
        private ConnectionStringType GetConnectionStringType()
        {
            ...
            if (_configFileProvider.LogDbEnabled)
            {
                ...
                {
                    throw new RomarrStartupException("Postgres MainDbConnectionString is set but LogDbConnectionString is not. Both must be set or neither.");
                }

                if (isLogDBConnectionStringSet && !isMainDBConnectionStringSet)
                {
                    throw new RomarrStartupException("Postgres LogDbConnectionString is set but MainDbConnectionString is not. Both must be set or neither.");
                }
            }

            if (isMainDBConnectionStringSet && _configFileProvider.PostgresHost.IsNotNullOrWhiteSpace())
            {
                throw new RomarrStartupException($"Either both Postgres connection strings must be set, or the other Postgres settings must be set, but not both.");
            }
