...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public string PostgresMainDb => _postgresOptions?.MainDb ?? GetValue("PostgresMainDb", "romarr-main", persist: false);
        public string PostgresLogDb => _postgresOptions?.LogDb ?? GetValue("PostgresLogDb", "romarr-log", persist: false);
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        private XDocument LoadConfigFile()
        {
            try
            {
                lock (Mutex)
                {
                    if (_diskProvider.FileExists(_configFile))
                    {
                        ...
                        {
                            throw new InvalidConfigFileException($"{_configFile} is empty. Please delete the config file and Romarr will recreate it.");
                        }

                        if (contents.All(char.IsControl))
                        {
                            throw new InvalidConfigFileException($"{_configFile} is corrupt. Please delete the config file and Romarr will recreate it.");
                        }
                        ...
                        if (config.Count != 1)
                        {
                            throw new InvalidConfigFileException($"{_configFile} is invalid. Please delete the config file and Romarr will recreate it.");
                        }
            ...
            {
                throw new InvalidConfigFileException($"{_configFile} is corrupt is invalid. Please delete the config file and Romarr will recreate it.", ex);
            }
        }
