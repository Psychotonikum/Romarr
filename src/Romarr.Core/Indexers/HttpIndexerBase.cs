...
using Romarr.Core.Http.CloudFlare;
...
namespace Romarr.Core.Indexers
{
        ...
        where TSettings : IIndexerSettings, new()
    {
        ...
        public override Task<IList<ReleaseInfo>> Fetch(SingleGameFileSearchCriteria searchCriteria)
        {
            if (!SupportsSearch)
            {
                return Task.FromResult<IList<ReleaseInfo>>(Array.Empty<ReleaseInfo>());
        ...
        public override Task<IList<ReleaseInfo>> Fetch(PlatformSearchCriteria searchCriteria)
        {
            if (!SupportsSearch)
            {
                return Task.FromResult<IList<ReleaseInfo>>(Array.Empty<ReleaseInfo>());
        ...
        public override Task<IList<ReleaseInfo>> Fetch(SpecialGameFileSearchCriteria searchCriteria)
        {
            if (!SupportsSearch)
            {
                return Task.FromResult<IList<ReleaseInfo>>(Array.Empty<ReleaseInfo>());
        ...
        protected virtual async Task<ValidationFailure> TestConnection()
        {
            ...
            catch (HttpException ex)
            {
                    ...
                    ex.Response.Content.Contains("not support the requested query"))
                {
                    ...
                    return new ValidationFailure(string.Empty, _localizationService.GetLocalizedString("IndexerValidationQueryPlatformGameFilesNotSupported"));
                }
