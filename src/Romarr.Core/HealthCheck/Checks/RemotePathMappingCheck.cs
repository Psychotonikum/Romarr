...
namespace Romarr.Core.HealthCheck.Checks
{
    ...
    public class RemotePathMappingCheck : HealthCheckBase, IProvideHealthCheck
    {
        ...
        public HealthCheck Check(IEvent message)
        {
            ...
            if (message is GameFileImportFailedEvent failureMessage)
            {
                // if we can see the file exists but the import failed then likely a permissions issue
                if (failureMessage.RomInfo != null)
                {
                    var gameFilePath = failureMessage.RomInfo.Path;

                    if (_diskProvider.FileExists(gameFilePath))
                    {
                        return new HealthCheck(GetType(),
                            HealthCheckResult.Error,
                            HealthCheckReason.RemotePathMappingDownloadPermissionsGameFile,
                            _localizationService.GetLocalizedString(
                                "RemotePathMappingDownloadPermissionsGameFileHealthCheckMessage",
                                new Dictionary<string, object>
                                {
                                    { "path", gameFilePath }
                                }),
                    ...
                    // ImportApprovedFiles and the file must have been removed part way through processing
                    return new HealthCheck(GetType(),
                        HealthCheckResult.Error,
                ...
                // If the previous case did not match then the failure occurred in DownloadedGameFileImportService,
                ...
                try
                {
                    ...
                    if (dlpath.IsNullOrWhiteSpace())
                    {
                        return new HealthCheck(
                            GetType(),
                            HealthCheckResult.Error,
                            HealthCheckReason.RemotePathMappingImportGameFileFailed,
                            _localizationService.GetLocalizedString("RemotePathMappingImportGameFileFailedHealthCheckMessage"),
