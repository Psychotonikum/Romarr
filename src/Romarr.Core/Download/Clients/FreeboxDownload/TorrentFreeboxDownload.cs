...
namespace Romarr.Core.Download.Clients.FreeboxDownload
{
    public class TorrentFreeboxDownload : TorrentClientBase<FreeboxDownloadSettings>
    {
        ...
        {
            if ((remoteRom.IsRecentGameFile() && Settings.RecentPriority == (int)FreeboxDownloadPriority.First) ||
                (!remoteRom.IsRecentGameFile() && Settings.OlderPriority == (int)FreeboxDownloadPriority.First))
            {
                return true;
            }
