...
namespace Romarr.Core.Download
{
    ...
    public class CompletedDownloadService : ICompletedDownloadService
    {
        ...
        public void Check(TrackedDownload trackedDownload)
        {
            ...
            var grabbedHistories = _historyService.FindByDownloadId(trackedDownload.DownloadItem.DownloadId).Where(h => h.EventType == FileHistoryEventType.Grabbed).ToList();
            var historyItem = grabbedHistories.MaxBy(h => h.Date);

            if (historyItem == null && trackedDownload.DownloadItem.Category.IsNullOrWhiteSpace())
            {
                trackedDownload.Warn("Download wasn't grabbed by Romarr and not in a category, Skipping.");
                return;
            }
            ...
            if (game == null)
            {
                ...
                Enum.TryParse(historyItem.Data.GetValueOrDefault(FileHistory.SERIES_MATCH_TYPE, SeriesMatchType.Unknown.ToString()), out SeriesMatchType seriesMatchType);
        ...
        public void Import(TrackedDownload trackedDownload)
        {
            ...
            var importResults = _downloadedGameFilesImportService.ProcessPath(outputPath,
                ImportMode.Auto,
            ...
            if (importResults.Count == 1)
            {
                ...
                if (firstResult.ImportDecision.Rejections.FirstOrDefault()?.Reason == ImportRejectionReason.MultiPlatform)
                {
            ...
            if (importResults.Any(c => c.Result != ImportResultType.Imported))
            {
                statusMessages.AddRange(
                    importResults
                        .Where(v => v.Result != ImportResultType.Imported && v.ImportDecision.LocalGameFile != null)
                        .OrderBy(v => v.ImportDecision.LocalGameFile.Path)
                        .Select(v =>
                            new TrackedDownloadStatusMessage(Path.GetFileName(v.ImportDecision.LocalGameFile.Path),
                                v.Errors)));
            }
        ...
        {
            var allGameFilesImported = importResults.Where(c => c.Result == ImportResultType.Imported)
                                                   .SelectMany(c => c.ImportDecision.LocalGameFile.Roms)
                                                   .Count() >= Math.Max(1,
            ...
            var grabbedHistory = historyItems.Where(h => h.EventType == FileHistoryEventType.Grabbed).ToList();
            ...
            // and an rom is removed, but later comes back with a different ID then Romarr will treat it as incomplete.
            ...
            var atLeastOneGameFileImported = importResults.Any(c => c.Result == ImportResultType.Imported);
            var allGameFilesImportedInHistory = _trackedDownloadAlreadyImported.IsImported(trackedDownload, historyItems);

            if (allGameFilesImportedInHistory)
            {
                ...
                var files = _mediaFileService.GetFiles(roms.Select(e => e.RomFileId).Where(i => i > 0).Distinct());
