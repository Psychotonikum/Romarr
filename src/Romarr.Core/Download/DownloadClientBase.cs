...
namespace Romarr.Core.Download
{
        ...
        where TSettings : IProviderConfig, new()
    {
        ...
        protected ValidationFailure TestFolder(string folder, string propertyName, bool mustBeWritable = true)
        {
            ...
            {
                return new RomarrValidationFailure(propertyName, "Folder does not exist")
                {
                    DetailedDescription = string.Format("The folder you specified does not exist or is inaccessible. Please verify the folder permissions for the user account '{0}', which is used to execute Romarr.", Environment.UserName)
                };
            }
            ...
            if (mustBeWritable && !_diskProvider.FolderWritable(folder))
            {
                ...
                return new RomarrValidationFailure(propertyName, "Unable to write to folder")
                {
                    DetailedDescription = string.Format("The folder you specified is not writable. Please verify the folder permissions for the user account '{0}', which is used to execute Romarr.", Environment.UserName)
                };
            }
