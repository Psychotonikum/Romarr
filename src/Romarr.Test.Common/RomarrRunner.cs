...
namespace Romarr.Test.Common
{
    public class RomarrRunner
    {
        ...
        public RomarrRunner(Logger logger, PostgresOptions postgresOptions, int port = 9797)
        {
        ...
        public void Start(bool enableAuth = false)
        {
            ...
            while (true)
            {
                ...
                {
                    Assert.Fail("Romarr failed to start within 120 seconds");
                }
                ...
                {
                    TestContext.Progress.WriteLine($"Romarr {Port} is started. Running Tests");
                    return;
                }

                TestContext.Progress.WriteLine("Waiting for Romarr to start. Response Status : {0}  [{1}] {2}", statusCall.ResponseStatus, statusCall.StatusDescription, statusCall.ErrorException.Message);
        ...
        public void Kill()
        {
            try
            {
                if (_nzbDroneProcess != null)
                {
                    ...
                    if (_nzbDroneProcess.HasExited)
                    {
                        var log = File.ReadAllLines(Path.Combine(AppData, "logs", "Romarr.trace.txt"));
        ...
        private void Start(string outputRomarrConsoleExe)
        {
            StringDictionary envVars = new();
            if (PostgresOptions?.Host != null)
            {
                envVars.Add("Romarr__Postgres__Host", PostgresOptions.Host);
                envVars.Add("Romarr__Postgres__Port", PostgresOptions.Port.ToString());
                envVars.Add("Romarr__Postgres__User", PostgresOptions.User);
                envVars.Add("Romarr__Postgres__Password", PostgresOptions.Password);
                envVars.Add("Romarr__Postgres__MainDb", PostgresOptions.MainDb);
                envVars.Add("Romarr__Postgres__LogDb", PostgresOptions.LogDb);

                TestContext.Progress.WriteLine("Using env vars:\n{0}", envVars.ToJson());
            }

            TestContext.Progress.WriteLine("Starting instance from {0} on port {1}", outputRomarrConsoleExe, Port);

            var args = "-nobrowser -nosingleinstancecheck -data=\"" + AppData + "\"";
            _nzbDroneProcess = _processProvider.Start(outputRomarrConsoleExe, args, envVars, OnOutputDataReceived, OnOutputDataReceived);
        }
