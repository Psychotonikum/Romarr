...
public class BackupController : Controller
{
    ...
    public ActionResult<object> RestoreUpload()
    {
        ...
        var path = Path.Combine(_appFolderInfo.TempFolder, $"romarr_backup_restore{extension}");
    ...
    private string GetBackupPath(Romarr.Core.Backup.Backup backup)
    {
        return Path.Combine(_backupService.GetBackupFolder(backup.Type), backup.Name);
    }

    private static int GetBackupId(Romarr.Core.Backup.Backup backup)
    {
        return HashConverter.GetHashInt31($"backup-{backup.Type}-{backup.Name}");
    }

    private Romarr.Core.Backup.Backup? GetBackupById(int id)
    {
