...
namespace Romarr.Api.V5.Queue
{
                               ...
                               IHandle<QueueUpdatedEvent>, IHandle<PendingReleasesUpdatedEvent>
    {
        ...
        public PagingResource<QueueResource> GetQueue([FromQuery] PagingRequestResource paging, bool includeUnknownSeriesItems = true, [FromQuery] int[]? gameIds = null, DownloadProtocol? protocol = null, [FromQuery] int[]? languages = null, [FromQuery] int[]? quality = null, [FromQuery] QueueStatus[]? status = null, [FromQuery] QueueSubresource[]? includeSubresources = null)
        {
            ...
            return pagingSpec.ApplyToPage((spec) => GetQueue(spec, gameIds?.ToHashSet() ?? [], protocol, languages?.ToHashSet() ?? [], quality?.ToHashSet() ?? [], status?.ToHashSet() ?? [], includeUnknownSeriesItems), (q) => MapToResource(q, includeSeries, includeGameFiles));
        }
        ...
        private void Remove(Romarr.Core.Queue.Queue pendingRelease, string? message, bool blocklist)
        {
            if (blocklist)
            {
        ...
        private QueueResource MapToResource(Romarr.Core.Queue.Queue queueItem, bool includeSeries, bool includeGameFiles)
        {
            return queueItem.ToResource(includeSeries, includeGameFiles);
        }
