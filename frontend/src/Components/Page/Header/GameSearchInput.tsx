  ...
  const handleKeyDown = useCallback(
    (event: KeyboardEvent<HTMLElement>) => {
      ...
      if (!suggestions.length || highlightedSectionIndex) {
        dispatch(
          push(
            `${window.Romarr.urlBase}/add/new?term=${encodeURIComponent(
              value
            )}`
          )
        );
      ...
      dispatch(
        push(
          `${window.Romarr.urlBase}/game/${selectedSuggestion.item.titleSlug}`
        )
      );
  ...
  const handleSuggestionSelected = useCallback(
    (
      ...
      if ('type' in suggestion) {
        ...
        setValue('');
        dispatch(
          push(`${window.Romarr.urlBase}/game/${suggestion.item.titleSlug}`)
        );
      }
    },
    [value, dispatch]
