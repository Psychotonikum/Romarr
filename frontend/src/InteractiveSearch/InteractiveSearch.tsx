...
function InteractiveSearch({ type, searchPayload }: InteractiveSearchProps) {
  ...
  const handleFilterSelect = useCallback(
    (selectedFilterKey: string | number) => {
      if (type === 'rom') {
        setReleaseOption('fileSelectedFilterKey', selectedFilterKey);
      } else {
        setReleaseOption('platformSelectedFilterKey', selectedFilterKey);
      }
    },
    [type]
  );
      ...
      {!isFetching && error ? (
        <div>
          {errorMessage ? (
            ...
            translate('FileSearchResultsLoadError')
          )}
        </div>
      ) : null}
