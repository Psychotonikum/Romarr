...
do
  case $c in
    ...
    m)   opt_mode=$OPTARG ;;
    r)   opt_report=1 ;;
    ?|h) printf "Usage: %s [-p] [-v mono-ver] [-m romarr|complete]\n" $0 
         printf " -p  run parallel\n"
         printf " -v  run specified mono version\n"
         printf " -m  run only mono-'complete' or 'romarr' package variants\n"
...
prepOne() {
    ...
    if [ "$opt_mode" != "romarr" ]; then
        docker build -t romarr-test-$MONO_VERSION --build-arg MONO_VERSION=$MONO_VERSION --build-arg MONO_URL=$MONO_URL --file mono/complete/Dockerfile mono
    fi

    if [ "$opt_mode" != "complete" ] && [ "$MONO_VERSION" != "5.0" ]; then    
        docker build -t romarr-test-$MONO_VERSION-romarr --build-arg MONO_VERSION=$MONO_VERSION --build-arg MONO_URL=$MONO_URL --file mono/romarr/Dockerfile mono
    fi
}
...
runOne() {
    ...
    if [ "$opt_mode" != "romarr" ]; then
        ...
        docker run $dockerArgs romarr-test-$MONO_VERSION
    fi
    ...
    if [ "$opt_mode" != "complete" ] && [ "$MONO_VERSION" != "5.0" ]; then   
        ...
        dockerArgs="$dockerArgs -v /${PWD}/../../_tests_linux:/data/_tests_linux:ro"
        dockerArgs="$dockerArgs -v /${PWD}/../../_output_linux:/data/_output_linux:ro"
        dockerArgs="$dockerArgs -v /${PWD}/../../_tests_results/mono-$MONO_VERSION-romarr:/data/_tests_results"
        dockerArgs="$dockerArgs --mount type=tmpfs,destination=//data/test,tmpfs-size=1g"
        docker run $dockerArgs romarr-test-$MONO_VERSION-romarr
    fi
