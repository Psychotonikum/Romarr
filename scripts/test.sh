...
FILES=( "Romarr.Api.Test.dll" "Romarr.Automation.Test.dll" "Romarr.Common.Test.dll" "Romarr.Core.Test.dll" "Romarr.Host.Test.dll" "Romarr.Integration.Test.dll" "Romarr.Libraries.Test.dll" "Romarr.Mono.Test.dll" "Romarr.Update.Test.dll" "Romarr.Windows.Test.dll" )
ASSMEBLIES=""
...
export ROMARR_TESTS_LOG_OUTPUT="File"
...
if [ "$PLATFORM" = "Windows" ]; then
  mkdir -p "$ProgramData/Romarr"
  WHERE="$WHERE&Category!=LINUX"
elif [ "$PLATFORM" = "Linux" ]; then
  mkdir -p ~/.config/Romarr
  WHERE="$WHERE&Category!=WINDOWS"
elif  [ "$PLATFORM" = "Mac" ]; then
  mkdir -p ~/Library/Application\ Support/Romarr
  WHERE="$WHERE&Category!=WINDOWS"
else
